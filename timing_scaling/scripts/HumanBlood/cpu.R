...
fit.devil <- devil::fit_devil(
  ...
  verbose = T,
  parallel.cores = 1,
  offset = 1e-6, max_iter = 100, init_overdispersion = 100
)
e <- Sys.time()
print(e-s)

devil.res <- devil::test_de(fit.devil, contrast = CONTRAST)
...
saveRDS(devil.final.res, paste0("results/HumanBlood/fits/cpu_devil_", n_genes, "_ngene_", n_cells, "_ncells_", N_CELL_TYPES, "_celltypes.rds"))

devil.res <- devil::test_de(fit.devil, contrast = CONTRAST, clusters = cls)
devil.final.res <- devil.res %>%
  cbind(fit.devil$beta) %>%
  cbind(dplyr::tibble(theta = fit.devil$overdispersion))
...
saveRDS(devil.final.res, paste0("results/HumanBlood/fits/cpu_devilS_", n_genes, "_ngene_", n_cells, "_ncells_", N_CELL_TYPES, "_celltypes.rds"))
rm(fit.devil, devil.res, devil.final.res)
...
for (n_genes in c(100, 1000, 5000)) {
  ...
  for (n_cells in c(1000, 100000, 1e6)) {
    ...
    res_name = paste0("results/HumanBlood/cpu_devil_", n_genes, "_ngene_", n_cells, "_ncells_", N_CELL_TYPES, "_celltypes.rds")
