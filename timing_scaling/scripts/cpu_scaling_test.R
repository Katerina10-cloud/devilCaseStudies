...
for (p_genes in c(.1, .5)) {
  ...
  for (p_cells in c(.1, .25, .5, .75)) {
    ...
    # c <- as.matrix(cnt[sub.genes.idx,sub.cell.idx])
...
saveRDS(devil.final.res, paste0("results/macaque_brain/fits/cpu_devil_", p_genes, "_pgene_", p_cells, "_pcells_", N_CELL_TYPES, "_celltypes.rds"))
